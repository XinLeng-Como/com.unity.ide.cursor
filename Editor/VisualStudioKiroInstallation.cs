    ...
    internal class VisualStudioKiroInstallation : VisualStudioInstallation
    {
        ...
        private static bool IsCandidateForDiscovery(string path)
        {
            ...
            return Directory.Exists(path)
                && Regex.IsMatch(path, ".*Kiro.*.app$", RegexOptions.IgnoreCase);
#elif UNITY_EDITOR_WIN
            return File.Exists(path)
                && Regex.IsMatch(path, ".*Kiro.*.exe$", RegexOptions.IgnoreCase);
#else
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    var manifest = JsonUtility.FromJson<VisualStudioCodeManifest>(
...
";
        ...
        {
            var normalizedTargetPath = solutionPath
                .Replace('\\', '/')
                .TrimEnd('/')
                .ToLowerInvariant();
            ...
            foreach (var process in processes)
            {
                try
                {
                    ...
                    if (workspaces != null && workspaces.Length > 0)
                    {
                        ...
                        {
                            var normalizedWorkspaceDir = workspace
                                .Replace('\\', '/')
                                .TrimEnd('/')
                                .ToLowerInvariant();
                            ...
                            if (
                                ...
                                || normalizedWorkspaceDir.StartsWith(
                                    normalizedTargetPath + "/",
                                    StringComparison.OrdinalIgnoreCase
                                )
                            )
